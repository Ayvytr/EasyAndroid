...
public class NumberPickerView extends View {
                ...
                switch(msg.what) {
                    case HANDLER_WHAT_REFRESH:
                        if(!mScroller.isFinished()) {
                            ...
                            Message changeMsg = getMsg(HANDLER_WHAT_LISTENER_VALUE_CHANGED, mPrevPickedIndex,
                                    willPickIndex, msg.obj);
        ...
        if(currPickedIndex != mPrevPickedIndex && mRespondChangeOnDetach) {
            try {
                if(mOnValueChangeListener != null) {
                    mOnValueChangeListener.onValueChange(NumberPickerView.this, mPrevPickedIndex + mMinValue,
                            currPickedIndex + mMinValue);
                }
                if(mOnValueChangeListenerRaw != null) {
                    mOnValueChangeListenerRaw
                            .onValueChangeRelativeToRaw(NumberPickerView.this, mPrevPickedIndex, currPickedIndex,
                                    mDisplayedValues);
                }
            } catch(Exception e) {
                e.printStackTrace();
    ...
    public int getRawContentSize() {
        if(mDisplayedValues != null) { return mDisplayedValues.length; }
        return 0;
    }
    ...
    private void respondPickedValueChanged(int oldVal, int newVal, Object respondChange) {
        ...
        if(oldVal != newVal) {
            if(respondChange == null || !(respondChange instanceof Boolean) || (Boolean) respondChange) {
                ...
                if(mOnValueChangeListenerRaw != null) {
                    mOnValueChangeListenerRaw
                            .onValueChangeRelativeToRaw(NumberPickerView.this, oldVal, newVal, mDisplayedValues);
                }
            }
        }
    ...
    private void scrollByIndexSmoothly(int deltaIndex, boolean needRespond) {
        ...
        if(duration < DEFAULT_MIN_SCROLL_BY_INDEX_DURATION) { duration = DEFAULT_MIN_SCROLL_BY_INDEX_DURATION; }
        if(duration > DEFAULT_MAX_SCROLL_BY_INDEX_DURATION) { duration = DEFAULT_MAX_SCROLL_BY_INDEX_DURATION; }
        ...
        if(specMode == MeasureSpec.EXACTLY) {
            ...
            int maxWidth = Math.max(mMaxWidthOfAlterArrayWithMeasureHint,
                    Math.max(mMaxWidthOfDisplayedValues, mMaxWidthOfAlterArrayWithoutMeasureHint)
                            + 2 * (gapOfHint + Math
                            .max(mWidthOfHintText, mWidthOfAlterHint) + marginOfHint + 2 * mItemPaddingHorizontal));
        ...
        for(int i = 0; i < mShowCount + 1; i++) {
            ...
            if(0 <= index && index < getOneRecycleSize()) {
                ...
                if(mTextEllipsize != null) {
                    str = TextUtils
                            .ellipsize(str, mPaintText, getWidth() - 2 * mItemPaddingHorizontal, getEllipsizeType());
                }
    ...
    private void updateMaxHeightOfDisplayedValues() {
        float savedTextSize = mPaintText.getTextSize();
        mPaintText.setTextSize(mTextSizeSelected);
        mMaxHeightOfDisplayedValues = (int) (mPaintText.getFontMetrics().bottom - mPaintText
                .getFontMetrics().top + 0.5);
        mPaintText.setTextSize(savedTextSize);
    }
